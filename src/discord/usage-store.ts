/**
 * Phase 35 (TOKEN-01, TOKEN-02): Per-turn token usage store for Discord agent.
 *
 * Appends usage records to date-partitioned JSONL files under
 * ~/.telemachus/discord-usage/YYYY-MM-DD.jsonl — same pattern as the
 * audit log in src/security/audit.ts.
 *
 * Phase 65 (HYG-03): mkdir→open→appendFile→datasync→close pipeline delegated
 * to the shared JsonlWriter class; public appendUsage signature is unchanged.
 *
...
export interface UsageRecord {
  ...
  userId: string      // Discord snowflake of the message author
...
/**
 * Returns the path to the discord-usage directory.
...
export function usageDir(): string {
  ...
  return join(home, '.telemachus', 'discord-usage')
}
